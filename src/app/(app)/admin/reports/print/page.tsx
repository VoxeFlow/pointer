...
import { reportService, fmtTime, fmtDate, fmtMinutes, type DailyRow } from "@/services/report-service";
...
export default async function PrintableReportPage(props: {
  ...
  const groupedRows = rows.reduce(
    (groups, row) => {
      ...
      if (!lastGroup || lastGroup.employeeName !== row.employeeName) {
        groups.push({ employeeName: row.employeeName, rows: [row] });
      } else {
    ...
    [] as Array<{ employeeName: string; rows: DailyRow[] }>,
  );
  ...
  return (
    <div className="bg-white min-h-screen font-sans text-black">
      ...
      <PrintActions employeeName={userId && rows.length > 0 ? rows[0].employeeName : undefined} />

      {/* Timestamp line -- hidden on print via CSS class */}
      <div className="no-print px-6 py-1 text-right text-xs text-gray-400 bg-gray-50 border-b">
            ...
            <table>
              ...
              <tbody>
                {groupedRows.map((group) => {
                  const employeeWorked = group.rows.reduce((acc, row) => acc + (row.workedMinutes ?? 0), 0);
                  const employeeExtra = group.rows.reduce((acc, row) => acc + (row.extraMinutes ?? 0), 0);
                  ...
                  return (
                    <Fragment key={group.employeeName}>
                      {group.rows.map((row, index) => (
                        ...
                        >
                          <td className="whitespace-nowrap font-medium">{fmtDate(row.date)}</td>
                      ...
                      <tr style={{ backgroundColor: "#171717", color: "white", fontWeight: "bold" }}>
                        <td
                          colSpan={!userId ? 7 : 6}
                          className="text-right pr-3 text-xs uppercase tracking-wider opacity-80"
                        >
                          Total de {group.employeeName}
                        </td>
                        <td className="text-center font-mono">{fmtMinutes(employeeWorked)}</td>
                        <td className="text-center font-mono text-green-300">
                          {employeeExtra > 0 ? `+${fmtMinutes(employeeExtra)}` : "—"}
                        </td>
                        <td></td>
                      </tr>
                    </Fragment>
              ...
              {userId ? (
                <tfoot>
                  <tr style={{ backgroundColor: "#171717", color: "white", fontWeight: "bold" }}>
                    <td colSpan={6} className="text-right pr-3 text-xs uppercase tracking-wider opacity-80">Total do período</td>
